...
with open("joint_knn.pkl", "rb") as f:
    knn = pickle.load(f)
...
def correct_keypoints(xy_2d, regressors):
    xy_flat = xy_2d.flatten().reshape(1, -1)  # (1, 34)
    xy_corrected = knn.predict(xy_flat)
    return xy_corrected.reshape(17, 2)
...
def run_evaluation(anno_dir, image_dir):
    ...
    for anno_file in anno_dir.glob("*.json"):
        ...
        for ann in data["annotations"]:
            ...
            xy_2d = correct_keypoints(xy_2d, knn)  
...
plt.tight_layout()
plt.savefig("SVC_knn_inference_cm.png", dpi=150, bbox_inches="tight")
plt.show()