...
def flip_keypoints(xy: np.ndarray) -> np.ndarray:
    xy_f = xy.copy()
    cx = (xy[:, 0].max() + xy[:, 0].min()) / 2.0
    xy_f[:, 0] = 2 * cx - xy[:, 0]
    for l, r in FLIP_PAIRS:
        xy_f[[l, r]] = xy_f[[r, l]]
    return xy_f

def rotate_keypoints(xy: np.ndarray, angle_deg: float) -> np.ndarray:
    cx, cy = xy[:, 0].mean(), xy[:, 1].mean()
    theta = np.radians(angle_deg)
    c, s = np.cos(theta), np.sin(theta)
    R = np.array([[c, -s], [s, c]])
    xy_c = xy - np.array([cx, cy])
    return (R @ xy_c.T).T + np.array([cx, cy])
...
def load_data(anno_dir, image_dir):
    ...
    for anno_file in anno_dir.glob("*.json"):
        ...
        for ann in data["annotations"]:
            ...
            if label != "teacher sitting":
                xy_f = flip_keypoints(xy_yolo)
                xy_fn = normalize_keypoints(xy_f, (x_min, y_min, w, h)).reshape(17, 2)
                X.append(build_features(xy_f, xy_fn))
                y.append(label)
                print(f"FLIP: {image_path.name} | {label}")

                for deg in [-10, 10]:
                    xy_r = rotate_keypoints(xy_yolo, deg)
                    x_min_r = xy_r[:, 0].min(); 
                    y_min_r = xy_r[:, 1].min()
                    w_r = xy_r[:, 0].max() - x_min_r
                    h_r = xy_r[:, 1].max() - y_min_r
                    xy_rn = normalize_keypoints(xy_r, (x_min_r, y_min_r, w_r, h_r)).reshape(17, 2)
                    X.append(build_features(xy_r, xy_rn))
                    y.append(label)
                    print(f"ROT {deg}: {image_path.name} | {label}")
...
plt.tight_layout()
plt.savefig("SVC_aug_cm.png", dpi=150, bbox_inches="tight")
plt.show()
...
best_model.fit(X, y)
with open("SVC_aug_feats.pkl", "wb") as f:
    pickle.dump(best_model, f)
print("Model saved as SVC_au_feats.pkl")
