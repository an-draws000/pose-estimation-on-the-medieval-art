...
with open("joint_regressors.pkl", "rb") as f:
...
def correct_keypoints(xy_2d, regressors):
    """xy_2d: (17, 2) normalized keypoints"""
    xy_corrected = xy_2d.copy()
    for j in range(17):
        context = np.delete(xy_corrected, j, axis=0).flatten()  # (32,)
        x_in = np.hstack([xy_corrected[j], context]).reshape(1, -1)  # (1, 34)
        xy_corrected[j] = regressors[j].predict(x_in)[0]
    return xy_corrected

# ── Main: IoU-matched predictions ────────────────────────────────────────────
...
def run_evaluation(anno_dir, image_dir):
    ...
    for anno_file in anno_dir.glob("*.json"):
        ...
        for ann in data["annotations"]:
            ...
            xy_2d = correct_keypoints(xy_2d, regressors)  
            xy_corrected_px = xy_2d.copy()
            xy_corrected_px[:, 0] = xy_2d[:, 0] * w + x_min
            xy_corrected_px[:, 1] = xy_2d[:, 1] * h + y_min
            features = build_features(xy_corrected_px, xy_2d)
...
plt.tight_layout()
plt.savefig("SVC_ridge_inference_cm.png", dpi=150, bbox_inches="tight")
plt.show()